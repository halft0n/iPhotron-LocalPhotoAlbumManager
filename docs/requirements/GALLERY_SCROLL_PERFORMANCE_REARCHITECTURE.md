...
3. 未加载数据必须立即显示稳定 placeholder，不允许从绘制调用栈触发同步加载。
...
Wheel / Scrollbar
  ...
  -> paint from in-memory snapshots or placeholders
...
- 不包含 `QImage`、`QPixmap` 或其他 GUI 线程绑定对象；
- 不复制完整 `metadata` 字典；
- 不包含 micro thumbnail BLOB；
...
禁止为了兼容通用 DTO 转换器而读取完整 metadata 或 micro thumbnail。Gallery 投影应有独立 row mapper，并通过查询计划和 benchmark 验证索引使用情况。
...
### 8.5 model 与 paint 的 placeholder 行为

`GalleryListModelAdapter.data()` 必须改为纯内存查询：

- row 已加载：返回稳定的 tile snapshot；
- row 未加载：立即返回稳定 placeholder；
- 不调用 `ensure_row_loaded()`；
- 不触发 SQLite、文件访问或图片解码；
- 可通过轻量、去重的缺失提示通知调度器，但通知不得阻塞。

placeholder 必须保持布局稳定，避免数据到达时 tile 尺寸变化引发布局抖动。
...
| hot | 滚动方向前方少量 tile | 高优先级预取 |
| warm | 更远但可能即将进入 viewport 的 tile | 低优先级，仅在 worker 空闲时执行 |
| stale | 已离开相关 generation 的 tile | 取消或丢弃 |

快速滚动时应缩小或跳过 warm 范围，优先保证 scrollbar 和 placeholder 绘制流畅。停止或减速后再补充高质量缩略图。
...
- `model.data()` miss 时立即返回 placeholder；
...
- `scrollContentsBy()` P95 不超过 `2ms`；
- 快速滚动时 scrollbar 可即时移动，即使 tile 暂时显示 placeholder。
...
- 禁止 Gallery window 同步复制完整 metadata 和解码 micro thumbnail；
...
- `model.data()` miss 返回稳定 placeholder，且不调用同步加载；
- `peek()` miss 不访问文件系统；
...
即使 worker 变慢，scrollbar 和 placeholder paint 仍必须保持响应；慢 worker 只能延迟内容完善，不能拖慢输入位移。
...
| placeholder 增多造成感知质量下降 | 快速滚动时短暂空白 | 稳定 placeholder、方向预取、停止后优先补齐 |
...
4. model placeholder 与 generation-aware window 发布。
...
- paint/model data miss 只返回稳定 placeholder；
